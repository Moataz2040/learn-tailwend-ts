...
export default function Sisebar() {
  ...
  return (
    <div className="flex">
      <div
        className={`bg-dark-pruple min-h-screen p-5 pt-8 duration-300 ${
          open ? "w-72" : "w-20"
        } relative z-50`}
      >
        <BsArrowRight
          className={`bg-white text-3xl rounded-full absolute -left-3 top-9 border border-dark-pruple
                cursor-pointer ${!open && "rotate-180"}`}
          onClick={() => {
            setOpen(!open);
          }}
        />
        {/* change condition of open to be as {open ? "rotate-180":""}
               - please download prettier extension and make it default formatter for vs code.. search on it firstly
                - change naming of folder in component folder to be lowercase 
                 - see the naming of exported component here and fix it 
                */}

        <div className="inline-flex">
          <FaLocationDot
            className={`bg-amber-500 text-4xl rounded cursor-pointer
                    block float-left ml-2 duration-500 p-1 ${
                      open && "rotate-[360deg]"
                    }`}
          />
          <div
              ...
              >
                ...
                {menu.submenu && open && (
                  <BsChevronDown
                    ...
                    className={`${submenuOpen && "rotate-180"} flex-1`}
                  />
                )}
              </li>
